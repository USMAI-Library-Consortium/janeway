...
def get_site_resources(request):
    ...
    if not press:
        ...
        if alias and alias.redirect:
            ...
            redirect_obj = redirect(alias.build_redirect_url(path=request.path))
        elif alias:
...
class SiteSettingsMiddleware(BaseMiddleware):
    ...
    def process_request(request):
        ...
        if journal is not None:
            ...
            request.model_content_type = ContentType.objects.get_for_model(journal)
            ...
            if settings.SITE_SEARCH_INDEXING_FREQUENCY and not settings.IN_TEST_RUNNER:
                request.site_search_docs_url = cms_logic.get_search_data_url(
                    request.press
                )
        ...
        if not request.is_secure():
            if (
                ...
                return redirect(
                    "https://{0}{1}".format(request.get_host(), request.path)
                )
...
class MaintenanceModeMiddleware(BaseMiddleware):
    ...
    def process_request(request):
        if request.journal is not None:
            maintenance_mode_setting = setting_handler.get_setting(
                "general", "maintenance_mode", request.journal
            )
            if maintenance_mode_setting and maintenance_mode_setting.processed_value:
                ...
                maintenance_mode_message = setting_handler.get_setting(
    ...
    def process_response(request, response):
        try:
            if not request.session.get("counter_tracking", None):
                ...
                request.session.get("counter_tracking", None)
                and request.resolver_match.url_name == "article_view"
            ):
